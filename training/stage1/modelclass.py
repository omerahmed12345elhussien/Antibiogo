...
* It also contains a custom training loop that implements exponential moving average on it, due to under flow with some cases.
...
class CustomModel(tf.keras.Model):
  def __init__(self, *args, **kwargs):
    ...
    self.meaniou_metric = tf.keras.metrics.MeanIoU(num_classes=2,name="iou",dtype=tf.float64)
...
class TrainingLoop():
  def __init__(self, model:tf.keras.Model, criterion, train_data_loader,val_data_loader,
    ...
    self.acc_metric = tf.keras.metrics.Accuracy(name="accuracy",dtype=tf.float64)
    self.meaniou_metric = tf.keras.metrics.MeanIoU(num_classes=2,name="iou",dtype=tf.float64)#,ignore_class=0)
  ...
  def _train_step(self,image, mask):
    ...
    with tf.GradientTape() as tape:
      ...
      # Compute the loss.
      loss_value = self.criterion(y_true=mask, y_pred=logits)
    # Use the gradient tape to automatically retrieve
    # the gradients of the trainable variables with respect to the loss.
  ...
  def _test_step(self,image, mask):
    ...
    # Compute the loss.
    loss_value = self.criterion(y_true=mask, y_pred=pred_mask)
    self.meaniou_metric.update_state(mask, tf.math.argmax(pred_mask, axis=-1))
    miou_value = self.meaniou_metric.result()
    self.meaniou_metric.reset_state()
    self.acc_metric.update_state(mask, tf.math.argmax(pred_mask, axis=-1))
